...
def get_frame_name():
    ...
    now = datetime.now()
    return now.strftime("%y%m%d%H%M%S_%f") + ".jpg"

def send_frame_data(sock, cam_no, frame_bytes, timestamp, filename):
    """Send frame data to Node.js via TCP"""
    try:
        metadata = {
            "camNo": cam_no,
            "timestamp": timestamp,
...
def main():
    ...
    if not cam.isOpened():
        ...
        log("Check: ls -l /dev/video* and verify camera is connected", "ERROR")
        return
    ...
    try:
        while True:
            ...
            # Encode to JPEG
            t2 = time.time()
            ret, jpeg_buffer = cv2.imencode('.jpg', frame, [int(cv2.IMWRITE_JPEG_QUALITY), JPEG_QUALITY])
            if not ret:
            ...
            # Maintain interval
            sleep_time = max(0, CAPTURE_INTERVAL - (time.time() - loop_start))
